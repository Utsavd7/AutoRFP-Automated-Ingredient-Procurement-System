...
The restore script clears only the validated disposable database. It restores the required least-privilege access without creating or changing passwords, then verifies access controls, data isolation, and recovery integrity. Any unsafe permission makes recovery fail closed.

After verification, the script clears the disposable database again, which makes consecutive monthly checks safe. A production restore requires a separate incident decision, an exact recovery point, owner approval, and a fresh backup of current production first; this automation intentionally cannot target production.
